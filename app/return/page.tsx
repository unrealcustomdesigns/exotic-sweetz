...
export default function ReturnPage() {
  ...
  const [destLocations, setDestLocations] = useState<Option[]>([]);
  ...
  useEffect(() => {
    async function load() {
      ...
      setDestLocations([...data.storageLocations, ...data.truckLocations]);
      setLoading(false);
    }
    load();
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formData.set('action', 'RETURN_FROM_STORE');
      formData.set('unitType', 'BOX');

      const fromLocId = formData.get('fromLocationId') as string;
      const storeLoc = storeLocations.find((l) => l.id === fromLocId);
      ...
      setSuccess(`Returned ${qty} box(es) from ${storeName}`);
  ...
  if (success) {
    return (
      <div className="text-center py-8">
        ...
        <h2 className="text-lg font-bold mb-1">Return Recorded!</h2>
  ...
  return (
    <div>
      <h1 className="page-header">↩️ Return from Store</h1>
      <p className="text-sm text-gray-500 -mt-3 mb-4">
        Move unsold boxes back from a store to storage or truck
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="label">Boxes Returning</label>
          <input name="quantity" type="number" className="input" min={1} defaultValue={1} required inputMode="numeric" />
        </div>

        <div>
          <label className="label">Notes (optional)</label>
          <textarea name="notes" className="input" rows={2} placeholder="Reason for return..." />
        </div>
